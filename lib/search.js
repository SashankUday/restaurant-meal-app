...
export function blockedIngredientMatches(dish, blockedIngredients) {
  if (!blockedIngredients?.length) return [];
  const dishIngredients = dish.ingredients || [];
  return blockedIngredients.filter((blocked) => (
    dishIngredients.some((ingredient) => normalise(ingredient).includes(normalise(blocked)))
  ));
}
...
export function passesFilters(dish, filters) {
  ...
  if (!filters?.showBlocked && blockedIngredientMatches(dish, filters?.blockedIngredients).length > 0) return false;
